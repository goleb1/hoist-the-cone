...
function LiveRefreshStrip({ report, lastCheckedAt, refreshError }: { report: ConeReport; lastCheckedAt: string; refreshError: string | null }) {
  const game = report.relevantGame;

  if (report.state !== "live" && report.state !== "scheduled") return null;
  ...
  return (
    <section className={`mb-5 rounded-[1.5rem] border px-4 py-3 shadow-xl shadow-black/10 sm:mb-6 sm:px-5 ${live ? "border-orange-400/50 bg-asphalt text-cream" : "border-orange-500/25 bg-orange-500/10 text-asphalt"}`}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <span className={`h-3 w-3 shrink-0 rounded-full ${live ? "animate-pulse bg-orange-500 shadow-[0_0_18px_rgba(255,106,0,0.9)]" : "bg-orange-500"}`} />
          <div className="min-w-0">
            <div className={`font-mono text-[0.62rem] font-black uppercase tracking-[0.22em] ${live ? "text-orange-200" : "text-orange-700"}`}>
              {live ? "Live game control" : "Pregame watch"}
            </div>
            <div className="mt-1 truncate text-sm font-black sm:text-base">
              {live ? `${score}${game?.inning ? ` · ${game.inning}` : ""}` : `Next refresh checks first pitch at ${game ? formatShortFirstPitch(game.date) : "game time"}`}
            </div>
          </div>
        </div>
        <div className={`font-mono text-[0.6rem] font-bold uppercase tracking-[0.16em] ${live ? "text-cream/65" : "text-asphalt/55"}`}>
          {refreshError ? "Refresh delayed" : `Auto-refresh ${live ? "15s" : "60s"} · ${formatGeneratedAt(lastCheckedAt)} ET`}
        </div>
      </div>
    </section>
  );
...
export function LiveConeDashboard({ initialReport }: { initialReport: ConeReport }) {
  ...
  const refreshMs = useMemo(() => {
    if (report.state === "live") return 15000;
    if (report.state === "scheduled") return 60000;
    return null;
  }, [report.state]);
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-cream text-asphalt">
      ...
      <div className="relative mx-auto w-full max-w-7xl px-3.5 py-4 sm:px-8 sm:py-5 lg:px-10">
        ...
        <LiveRefreshStrip report={report} lastCheckedAt={lastCheckedAt} refreshError={refreshError} />
